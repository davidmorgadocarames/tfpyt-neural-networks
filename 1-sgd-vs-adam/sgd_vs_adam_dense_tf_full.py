"""
SGD vs Adam (Keras) sobre MNIST completo, mini-batch (batch_size=128, ~422 actualizaciones de
pesos por epoca) -- equivalente TF de
numpy-neural-networks-from-scratch/07-reconocimiento-digitos/sgd_vs_adam_full.py.

Misma arquitectura y mismo dataset que 01_dense_classifier_mnist.py (60.000 train / 10.000
test, validation_split=0.1 como ya hace ese script) -- no lo toca, guarda sus propios
resultados en results_sgd_vs_adam/dense_tf_minibatch/.

Uso: python sgd_vs_adam_dense_tf_full.py
"""
...
RESULTS_DIR = Path(__file__).parent / "results_sgd_vs_adam" / "dense_tf_minibatch"
...
def cargar_datos(seed_split, quiet=False):
    """Split estratificado 80/20 de train (para reservar validacion propia, no
    validation_split de Keras) mas el test estandar de MNIST -- seed_split decide que
    imagenes de train caen en validacion. A diferencia de la escala full-batch no se
    resamplea el dataset: se usan los 60.000/10.000 completos, solo cambia qué 10% de train
    se aparta para validar."""
...
def graficar_curva_comparativa(curvas_val, ruta_salida):
    ...
    plt.yscale("log")
    plt.title("SGD vs Adam (Keras): perdida de validacion por epoca (MNIST completo, mini-batch)",
               fontweight="bold")
    plt.xlabel("Epoca")
...
def main(seed_split=42, seed_modelo=42, quiet=False, guardar_graficas=True) -> dict:
    ...
    for nombre, crear_optimizador in VARIANTES:
        if not quiet:
            print(f"\n=== Entrenando '{nombre}' (Keras, mini-batch) ===")
