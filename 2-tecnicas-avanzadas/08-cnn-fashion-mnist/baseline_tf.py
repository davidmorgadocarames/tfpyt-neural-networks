"""
Baseline (Keras) de la CNN sobre Fashion-MNIST -- el script "main" de este problema. Importa
las constantes de adam.py/lr_decay.py/weight_decay.py y monta UN optimizador AdamW con
LR-schedule y weight decay ya combinados -- ver ../03-tipos-clientes/baseline_tf.py para la
explicacion completa del diseno.

Como 07-reconocimiento-digitos, es una de "las redes convencionales": Fashion-MNIST completo
(60k/10k), mini-batch (batch_size=128), y BatchNorm integrado en la arquitectura:
Conv2D(32) -> BatchNorm -> ReLU -> MaxPool -> Conv2D(64) -> BatchNorm -> ReLU -> MaxPool ->
Flatten -> Dense(128) -> BatchNorm -> ReLU -> Dense(10, softmax) -- mismo orden que
RRNN/08-cnn-fashion-mnist/batchnorm.py (BatchNorm tras cada Conv y tras la primera densa).
...
def cargar_datos(seed_split, quiet=False):
    """Split 90/10 de train (para reservar validacion propia) + el test estandar de
    Fashion-MNIST."""
...
def build_model() -> tf.keras.Model:
    """BatchNormalization usa momentum=0.9 (en vez del 0.99 por defecto de Keras) -- verificado
    con ensembling_kfold_tf.py (solo 30 steps/epoca sobre la muestra reducida) que con el
    momentum por defecto la media movil tarda demasiados steps en converger (0.99**30 todavia
    conserva un 74% del estado inicial), y con 3 BatchNorm apiladas el desajuste train/inferencia
    se agrava a cada capa: la red memoriza el train set (accuracy ~1.0) mientras la accuracy de
    validacion se desploma (~35%, verificado). momentum=0.9 iguala el comportamiento por defecto
    de PyTorch (convencion opuesta: su momentum=0.1 equivale a un momentum=0.9 aqui) y corrige el
    problema (validado: recupera ~88% de accuracy en el mismo escenario)."""
