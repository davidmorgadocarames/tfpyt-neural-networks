"""
Ensembling + validacion cruzada k-fold combinados (Keras) sobre zonas de espirales -- mismo
diseno que ../03-tipos-clientes/ensembling_kfold_tf.py (split 80/20 train_pool/test_final
held-out, K=5 folds estratificados, los K modelos se conservan y se promedian sus
predicciones). Cada modelo usa la config combinada de baseline_tf.py.
...
def main(quiet=False) -> dict:
    ...
    X_test_final, Y_test_final = X[idx_test_final], Y_num[idx_test_final]
    ...
    plt.title(f"Ensembling + k-fold (k={K_FOLDS}) -- zonas de espirales (Keras)", fontweight="bold")
