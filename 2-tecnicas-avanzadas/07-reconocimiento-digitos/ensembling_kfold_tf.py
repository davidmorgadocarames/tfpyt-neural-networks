"""
Ensembling + validacion cruzada k-fold combinados (Keras) sobre digitos MNIST -- mismo diseno
que ../03-tipos-clientes/ensembling_kfold_tf.py (split 80/20 train_pool/test_final held-out,
K=5 folds estratificados, los K modelos se conservan y se promedian sus predicciones). Cada
modelo usa la config combinada de baseline_tf.py (AdamW+LRdecay+WeightDecay+BatchNorm).

Usa una muestra reducida y estratificada (600/digito = 6000 imagenes, misma escala que las
variantes full-batch de ../../1-sgd-vs-adam/) en vez de las 60k completas -- entrenar 5 modelos
con mini-batch sobre el dataset completo hubiera sido demasiado lento para una demostracion de
la tecnica.
...
def muestrear_mnist(quiet=False):
    ...
    for digito in range(10):
        ...
        idx_muestra.extend(idx_digito[:N_POR_DIGITO])
    idx_muestra = np.array(idx_muestra)
    rng.shuffle(idx_muestra)

    if not quiet:
        print(f"MNIST muestreado: {len(idx_muestra)} imagenes ({N_POR_DIGITO}/digito)")
...
def main(quiet=False) -> dict:
    X, Y = muestrear_mnist(quiet=quiet)
    ...
    plt.title(f"Ensembling + k-fold (k={K_FOLDS}) -- MNIST (Keras)", fontweight="bold")
